"""
generate_hc_within_cartoon.py
==============================
Within-cartoon DySCo timecourse for all HC participants (ses-01).
Mirrors adult generate_adult_within_cartoon.py style.
Saved to: D:/adult_controls/Adult/dysco_results/group_average/within_cartoon_per_patient/
"""
...
OUT_DIR = os.path.join(HC_ROOT, "group_average", "within_cartoon_per_patient")
...
def _plot_participant(pid_short, cartoon_files):
    ...
    for fpath in cartoon_files:
        try:
            ...
            print(f"  Could not load {os.path.basename(fpath)}: {e}"); continue
    ...
    if not en_list:
        print(f"  [{pid_short}] No data — skipping"); return
    ...
    fig.suptitle(
        ...
        f"Average across {len(cartoon_files)} ses-01 cartoon run(s)  |  "
...
for dname in pid_dirs:
    ...
    if not cartoon_files:
        print(f"  [{pid_short}] No ses-01 cartoon files — skipping"); continue
