"""
generate_adult_group_boxplot.py
================================
Three-condition boxplot (Video / Please Wait / Rest) for all adult
participants, styled to match the DELFT_NEW reference figure.

Video and Please Wait windows are extracted from cartoon runs using the
Shamshiri et al. (2016) block boundaries.  Rest = all rest-run windows.
One dot per participant = participant median.

Run:  python generate_adult_group_boxplot.py
"""
...
# Shamshiri et al. (2016) — same paradigm as DELFT_NEW
VIDEO_BLOCKS_VOL = [(0, 111), (150, 261)]
WAIT_BLOCKS_VOL  = [(111, 150), (261, 300)]   # adult has 300 TRs
...
def _win_condition_en(i):
    """Map entropy/norm2 window i → condition using centre volume."""
    c = i + HALF_WIN
...
def _win_condition_spd(i):
    """Map speed window i → condition (centre shifted by LAG)."""
    c = i + HALF_WIN + LAG
    if any(s <= c < e for s, e in VIDEO_BLOCKS_VOL): return "video"
    if any(s <= c < e for s, e in WAIT_BLOCKS_VOL):  return "wait"
    return None
...
def _is_cartoon(fname):
    return bool(re.search(r"task-cartoon", fname))

def _is_rest(fname):
    return bool(re.search(r"task-rest", fname))
...
# group_pools[metric][condition]  = flat list of all window values
group_pools = {m: {c: [] for c in CONDITIONS} for m, _ in METRICS}

# part_pools[pid][metric][condition] = values for this participant
part_pools = {}
...
for dname in pid_dirs:
    ...
    if any(pools[m][c] for m, _ in METRICS for c in CONDITIONS):
        ...
        n_runs = len(npy_files)
        print(f"  {pid_short}: {n_runs} run(s) loaded")
    else:
...
save_path = os.path.join(OUTPUT_ROOT, "adult_group_boxplot_by_condition.png")
